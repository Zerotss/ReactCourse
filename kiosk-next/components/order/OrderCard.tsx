            ...
            <dl className="mt-6 space-y-4">
                {order.orderProducts.map(orderProduct => (
                    <div key={orderProduct.id}
                        ...
                        <dt className="flex items-center text-sm text-gray-600"><span className="font-black">({orderProduct.quantity}){''} </span></dt>
                        <dd className="text-sm font-medium text-gray-900"><span>{orderProduct.product.name}</span></dd>
                    </div>
                ))}
